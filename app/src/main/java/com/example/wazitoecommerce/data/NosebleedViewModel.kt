...
class NosebleedViewModel(var navController:NavHostController, var context: Context) {
        ...
        storageRef.putFile(filePath).addOnCompleteListener{
            ...
            if (it.isSuccessful){
                ...
                storageRef.downloadUrl.addOnSuccessListener {
                    ...
                    var ins = nosebleed(instruction, instruction1, instruction2,imageUrl,InstructionId)
    ...
    fun allInstructions(
        ...
        Instructions:SnapshotStateList<nosebleed>
    ):SnapshotStateList<nosebleed>{
        progress.show()
        var ref = FirebaseDatabase.getInstance().getReference().child("nosebleeds")
        ref.addValueEventListener(object: ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                Instructions.clear()
                for (snap in snapshot.children){
                    var value = snap.getValue(nosebleed::class.java)
